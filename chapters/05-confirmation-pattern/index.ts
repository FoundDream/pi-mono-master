...
function createSendEmailTool(
  ...
  return {
    name: 'send_email',
    label: 'Send Email',
    description: 'Send an email to the given address. Requires user confirmation.',
    parameters: Type.Object({
    ...
    execute: async (_toolCallId, params) => {
      ...
      if (!confirmed) {
        console.log('   ❌ Cancelled by user\n')
        return {
          content: [{ type: 'text' as const, text: 'User cancelled the email.' }],
          details: {},
        }
      }
...
const question = process.argv[2] || 'Delete the file /tmp/old-backup.log because it is outdated'

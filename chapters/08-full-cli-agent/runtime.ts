...
export class AgentRuntime {
  ...
  private async initSession(sm: SessionManager) {
    ...
    const tools = this.config.includeCodingTools
  ...
  createConfirmationWaiter() {
    return this.confirmation.waiter;
  }
  confirmTool() {
  ...
  async openSession(sessionPath: string) {
    this.session.dispose();
    await this.initSession(
      SessionManager.open(sessionPath, this.config.sessionDir),
    );
  ...
  async continueRecentSession() {
    ...
    try {
      await this.initSession(
        SessionManager.continueRecent(this.config.cwd, this.config.sessionDir),
      );

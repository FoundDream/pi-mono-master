...
export const weatherTool: ToolDefinition = {
  ...
  execute: async (_toolCallId, params) => {
    ...
    const data: Record<string, { temp: string; condition: string; humidity: string }> = {
    ...
    const weather = data[key] || { temp: '20°C', condition: 'Clear', humidity: '50%' }
    return {
...
export function createDangerousTool(
  ...
  return {
    ...
    parameters: Type.Object({
      operation: Type.String({ description: 'Description of the operation to perform' }),
      reason: Type.String({ description: 'Why this operation is needed' }),
    }),
    execute: async (_toolCallId, params) => {
      const { operation, reason } = params as { operation: string; reason: string }
      ...
      if (!confirmed) {
        ...
        return {
          content: [{ type: 'text' as const, text: 'User cancelled the operation.' }],
          details: {},
        }
      }
      ...
      return {
        content: [{ type: 'text' as const, text: `Operation completed: ${operation}` }],
        details: {},
      }
    },
  }
